...
from market_health.providers.calendar_provider import DEFAULT_CONFIG_PATH, load_calendar_provider  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--print", dest="do_print", action="store_true", help="Print normalized calendar.v1 JSON")
    args = ap.parse_args()
    ...
    if args.status and not args.do_print:
        print(f"status={b.status} events={len(b.events)} config={os.path.expanduser(args.config)}")
        return 0
    ...
    out = {
        ...
        "source": b.source,
        "events": [
            {"ts": ev.ts, "symbol": ev.symbol, "kind": ev.kind, "label": ev.label, "extra": ev.extra}
            for ev in b.events
        ],
        "errors": b.errors,
    }
