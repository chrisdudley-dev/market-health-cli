...
from market_health.providers.flow_provider import (
    DEFAULT_FLOW_CONFIG,
    load_flow_provider,
)
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Export flow.v1.json to ~/.cache/jerboa (offline, provider boundary)."
    )
    ap.add_argument("--out", default=os.path.expanduser("~/.cache/jerboa/flow.v1.json"))
    ...
    prov = load_flow_provider(os.path.expanduser(str(args.config)))
    batch = prov.get_flow(symbols)
    ...
    if not args.quiet:
        print(
            f"OK: wrote {out_p} status={batch.status} points={len(batch.points)} provider={prov.describe()}"
        )
    return 0
