...
def _is_market_session_fresh(value: Any, max_age_minutes: int = 15):
    ...
    ttl = timedelta(minutes=int(max_age_minutes or 0))

    # Pull a small NYSE schedule window around "now".
    ...
    opens = [ts.to_pydatetime().astimezone(timezone.utc) for ts in sched["market_open"]]
    closes = [
        ts.to_pydatetime().astimezone(timezone.utc) for ts in sched["market_close"]
    ]

    # Regular session currently open -> rolling TTL from now.
    for open_ts, close_ts in zip(opens, closes):
        if open_ts <= now_utc <= close_ts:
            return (now_utc - dt) <= ttl

    # Market currently closed -> anchor freshness to the most recent regular close.
    prior_closes = [c for c in closes if c <= now_utc]
    if not prior_closes:
        return False
    last_close = prior_closes[-1]
    return dt >= (last_close - ttl)
...
def _is_same_or_last_completed_session(value: Any):
    ...
    dt_session = dt.astimezone(timezone.utc).date().isoformat()

    # If a regular session is live right now, require data from this same session.
    for session_label, open_ts, close_ts in rows:
        if open_ts <= now_utc <= close_ts:
            return dt_session == session_label

    # If market is closed, allow data from the most recently completed session.
...
def _intraday_fresh_or_last_completed_session(value, max_age_minutes=15):
    ...
    try:
        ...
        # Graceful fallback when market-calendar dependency is unavailable.
        # Keep intraday TTL behavior during live weekday hours, and otherwise
        # treat the last completed weekday session as fresh.
        try:
    ...
    # Live session: same session + within TTL
    for session_label, open_ts, close_ts in rows:
        if open_ts <= now_utc <= close_ts:
    ...
    # Closed market: last completed session is still fresh
    prior_rows = [r for r in rows if r[2] <= now_utc]
    if not prior_rows:
        return False
