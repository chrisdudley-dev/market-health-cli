...
def compute_d_checks(
    *,
    ...
    return [
        d1_volatility_trend(horizon_days=horizon_days, atrp14=atrp14,
        ...
        d2_tail_gap_risk(horizon_days=horizon_days, returns=returns, calendar=calendar, atrp14=atrp14),
        ...
        d5_drawdown_vulnerability(horizon_days=horizon_days, close=close, lo20=lo20, atrp14=atrp14),
        d6_risk_reward_feasibility(horizon_days=horizon_days, atrp14=atrp14, support_cushion_proxy=support_cushion_proxy, corr20=corr20
        ),
    ]
...
def d4_liquidity_stress(
    *, horizon_days: int, returns: Optional[Sequence[Optional[float]]], volume: Optional[Sequence[Number]]
) -> ForecastCheck:
    # horizon-derived intermediate (ensures horizon_days is used in this check)
    h = int(horizon_days) if int(horizon_days) > 0 else 1
    h_window = int(round(10 * (h ** 0.5)))
...
def d5_drawdown_vulnerability(
    *, horizon_days: int, close: Optional[float], lo20: Optional[float], atrp14: Optional[float]
) -> ForecastCheck:
    # horizon-derived intermediate (ensures horizon_days is used in this check)
    h = int(horizon_days) if int(horizon_days) > 0 else 1
    h_window = int(round(10 * (h ** 0.5)))
