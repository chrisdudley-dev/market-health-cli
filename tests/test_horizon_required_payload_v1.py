...
def test_all_checks_use_horizon_and_hash_diff():
    doc = json.loads(
        Path("tests/fixtures/golden.forecast_scores.v1.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    for sym, by_h in sym_map.items():
        ...
        for dim in dims:
            ...
            for i in range(6):
                ...
                assert a.get("horizon_days") == 1, (
                    f"{sym} {dim}{i + 1}: missing/incorrect horizon_days for H1"
                )
                assert b.get("horizon_days") == 5, (
                    f"{sym} {dim}{i + 1}: missing/incorrect horizon_days for H5"
                )
                ...
                assert _h(a) != _h(b), (
                    f"{sym} {dim}{i + 1}: check hash unexpectedly identical across horizons"
                )
