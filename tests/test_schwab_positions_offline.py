...
def test_offline_importer_produces_valid_positions_v1(tmp_path):
    ...
    _run([sys.executable, "scripts/import_positions_schwab_json.py",
          "--in", str(SAMPLE),
          "--out", str(out)])

    out_text = _run([sys.executable, "scripts/validate_positions_v1.py",
                     "--path", str(out)])
...
def test_refresh_positions_cache_supports_schwab_json(tmp_path):
    ...
    _run(["bash", "scripts/cache/refresh_positions_cache.sh",
          "--schwab-json", str(SAMPLE),
          "--out", str(out)])
