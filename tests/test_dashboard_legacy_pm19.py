...
def test_render_reco_shows_watch_levels_structure_widget(monkeypatch, tmp_path):
    fs_doc = {
    ...
    assert "Watch Levels / Structure — XLE" in out
    assert "support" in out
    assert "resistance" in out
    assert "breakout" in out
    assert "breakdown" in out
    assert "reclaim" in out
    assert "cushion" in out
    assert "overhead" in out
    assert "82.10 / 82.60 / 83.00" in out
    assert "88.40 / 88.90 / 89.30" in out
    assert "89.30" in out
    assert "82.10" in out
    assert "83.00" in out
    assert "overhead_heavy, breakout_ready" in out
